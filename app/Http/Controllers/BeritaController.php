...
class BeritaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($berita->save()) {
            ...
            return redirect()->back()->with('error', 'Terjadi kesalahan saat menambahkan berita.');
        }
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $filename = $berita->gambar; // default: pakai foto lama
        ...
        if ($berita->save()) {
            $galeriTerkait = Galeri::where('berita_id', $berita->id)->first();
            $tetapDiGaleri = $request->has('masukkan_galeri') && $request->masukkan_galeri == '1';

            if ($tetapDiGaleri) {
                // updateOrCreate: kalau sudah ada relasinya, update (termasuk foto & teks).
                // Kalau belum ada (baru sekarang admin centang), buat baru.
                Galeri::updateOrCreate(
                    ...
                    [
                        ...
                        'deskripsi'   => Str::limit(strip_tags($request->konten), 100),
                        'kategori_id' => $request->kategori_id,
                        'gambar'      => $filename, // otomatis sinkron setiap kali update
                    ]
                );
            } elseif ($galeriTerkait) {
                // Admin SENGAJA uncheck (bukan lupa, karena checkbox default sudah pre-filled sesuai kondisi asli)
                // Hapus HANYA baris galeri, JANGAN hapus file fisik — file masih dipakai berita
                $galeriTerkait->delete();
            }
    ...
    public function destroy(string $id)
    {
        ...
        // Hapus baris galeri terkait dulu (kalau ada) — hanya hapus row DB,
        // JANGAN hapus file di sini karena file yang sama akan dihapus di bawah (hindari delete ganda)
        $galeriTerkait = Galeri::where('berita_id', $berita->id)->first();
        if ($galeriTerkait) {
            $galeriTerkait->delete();
        }
