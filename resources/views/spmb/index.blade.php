      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        ...
        <div data-aos="fade-up">
          <div class="lg:sticky lg:top-24">
            @if ($spmb && $spmb->gambar)
              <img src="{{ asset('storage/spmb/' . $spmb->gambar) }}" alt="Poster {{ $spmb->nama }}" class="w-full h-auto rounded-2xl shadow-xl shadow-slate-200 border border-gray-100">
            @else
              <div class="w-full aspect-3/4 rounded-2xl bg-gray-100 border border-dashed border-gray-300 flex items-center justify-center text-gray-400">
                <div class="text-center">
                  <i class="fa-solid fa-image text-4xl mb-3"></i>
                  <p class="text-sm font-medium">Poster belum tersedia</p>
                </div>
              </div>
            @endif
          </div>
        ...
        <div>
          ...
          <p class="text-gray-500 mb-10 leading-relaxed">
            Ikuti langkah-langkah berikut untuk mendaftar sebagai peserta didik baru di SMK Senopati.
          </p>
          ...
          @if ($spmb && $spmb->kontak)
            <a href="https://wa.me/{{ $spmb->kontak }}?text={{ urlencode('Halo Admin SPMB SMK Senopati, saya ingin bertanya tentang pendaftaran peserta didik baru.') }}" target="_blank"
              rel="noopener noreferrer" class="inline-flex items-center gap-2 mt-10 px-6 py-3 rounded-full bg-primary hover:bg-accent text-white font-black shadow-lg shadow-blue-200 transition-colors">
          ...
          @else
            <div class="mt-10 px-6 py-3 rounded-full bg-gray-100 text-gray-400 font-black inline-flex items-center gap-2 cursor-not-allowed">
